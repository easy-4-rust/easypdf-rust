...
pub(super) fn build_cms_signed_data(
    ...
    let signer_info = der_seq(&concat(&[
        &der_int(&[1u8]),
        &issuer_and_serial,
        &digest_alg,
        &sig_alg,
    ...
    let signed_data = der_seq(&concat(&[
        &der_int(&[1u8]),
        &digest_algs,
        &encap_content,
        &certs,
...
pub(super) fn parse_cms_signed_data(der: &[u8]) -> Result<ParsedCms, CryptoError> {
    ...
    if content_type != OID_SIGNED_DATA_VAL {
        return Err(CryptoError::InvalidSignedPdf(
            "CMS content type is not signedData".into(),
        ));
    }
    let (tag, inner) = ci
        .read_tlv()
        .map_err(|_| CryptoError::InvalidSignedPdf("missing CMS content".into()))?;
    if tag != 0xA0 {
        return Err(CryptoError::InvalidSignedPdf(
            "expected [0] EXPLICIT tag for SignedData".into(),
        ));
    }
    ...
    if version != [1u8] {
        return Err(CryptoError::InvalidSignedPdf(format!(
            "unsupported SignedData version: {version:?}"
        )));
    }
    let _ = s.read_set()?;
    ...
    let certs_der = s
        .read_ctx_implicit(0)?
        .ok_or_else(|| CryptoError::InvalidSignedPdf("CMS missing certificates".into()))?;
    let mut cert_reader = DerReader::new(certs_der);
    let (cert_tag, cert_value) = cert_reader.read_tlv()?;
    if cert_tag != 0x30 {
        return Err(CryptoError::InvalidSignedPdf(
            "expected SEQUENCE for certificate in CMS".into(),
        ));
    }
    ...
    if si_version != [1u8] {
        return Err(CryptoError::InvalidSignedPdf(format!(
            "unsupported SignerInfo version: {si_version:?}"
        )));
    }
    let _sid = si.read_sequence()?;
    let _ = si.read_sequence()?;
    if si.peek_tag() == Some(0xA0) {
        si.skip_field()?;
    }
    let _ = si.read_sequence()?;
    let signature = si.read_octet_string()?.to_vec();
    Ok(ParsedCms {
        certificate_der: first_cert,
        signature,
    })
}
...
pub(super) fn parse_x509_cert(cert_der: &[u8]) -> Result<(CertificateInfo, Vec<u8>), CryptoError> {
    ...
    let info = CertificateInfo {
        cn,
        issuer,
        not_before: format!("{}", validity.not_before),
        not_after: format!("{}", validity.not_after),
    };
...
pub(super) fn parse_private_key(der: &[u8]) -> Result<ring::signature::RsaKeyPair, CryptoError> {
    ...
    ring::signature::RsaKeyPair::from_der(der)
        .map_err(|e| CryptoError::InvalidKey(format!("RSA key parse error: {e}")))
}
